...
static long asmlinkage new_sys_mkdir(const char __user *pathname, umode_t mode) {
    /*
   ...
   此函数将from指针指向的用户空间地址开始的连续n个字节的数据产送到to指针指向的内核空间地址
     */
    char pathName[1016];
    memset(pathName, 0, 1016);
    if (!copy_from_user(pathName, pathname, strnlen_user(pathname, 1016))) {
